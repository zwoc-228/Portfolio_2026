# UI reference and tool links

1. Dine — interface hierarchy / restrained premium product presentation
   https://dinehq.com/

2. Codrops — Building an Endless Interactive Glass Xylophone with Three.js
   https://tympanus.net/codrops/2026/08/04/building-an-endless-interactive-glass-xylophone-with-three-js/

3. Codrops — Building an Infinite Liquid Glass Grid with Three.js, WebGPU, and TSL
   https://tympanus.net/codrops/2026/09/08/building-an-infinite-liquid-glass-grid-with-three-js-webgpu-and-tsl/

4. Codrops — Creating the Effect of Transparent Glass and Plastic in Three.js
   https://tympanus.net/codrops/2021/10/27/creating-the-effect-of-transparent-glass-and-plastic-in-three-js/

5. Codrops — Building Seamless 3D Transitions with Webflow, GSAP, and Three.js
   https://tympanus.net/codrops/2026/03/18/building-seamless-3d-transitions-with-webflow-gsap-and-three-js/

6. MDN — backdrop-filter
   https://developer.mozilla.org/en-US/docs/Web/CSS/backdrop-filter

7. Three.js — MeshPhysicalMaterial
   https://threejs.org/docs/#api/en/materials/MeshPhysicalMaterial

8. Three.js — WebGLRenderer
   https://threejs.org/docs/#api/en/renderers/WebGLRenderer

9. Three.js — Responsive rendering
   https://threejs.org/manual/#en/responsive

10. GSAP — quickTo (reviewed for high-frequency pointer animation patterns; not added as a dependency)
    https://gsap.com/docs/v3/GSAP/gsap.quickTo%28%29/
