...
// Round 30 moves the blur cost out of the fullscreen floor material: instead of 25 reflection
// texture reads for every floor pixel on every frame, the 1024x576 reflection is blurred only
// when the reflection actually changes, then the floor samples the finished texture once.
export function createFloorReflection(renderer,scene,ground){
 ...
 const uniforms={floorReflection:{value:blurB.texture},floorProjection:{value:matrix},glowWorldXZ:{value:new T.Vector2(0,0)},glowStrength:{value:0},glowRadius:{value:.36},uiCenter:{value:new T.Vector2(0,0)},uiAxis:{value:new T.Vector2(1,0)},uiHalfLength:{value:0},uiWidth:{value:.18},uiStrength:{value:0}};
 ...
 ground.material.customProgramCacheKey=()=> 'reference-floor-v9-preblurred';
 ...
 return {
  ...
  setUICaustic(x,z,ax,az,halfLength,width,strength){uniforms.uiCenter.value.set(x,z);uniforms.uiAxis.value.set(ax,az);uniforms.uiHalfLength.value=halfLength;uniforms.uiWidth.value=width;uniforms.uiStrength.value=strength;},
  update(camera){
   ...
   ground.visible=false;scene.background=null;renderer.setClearColor(0x000000,0);renderer.setRenderTarget(raw);renderer.clear();renderer.render(scene,mirror);
   blur();
